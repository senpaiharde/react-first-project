...
function saveBook(book) {
    ...
    const existingBookIndex = allBooks.findIndex((b)=> b.id === book.id);
    if(existingBookIndex !== -1){
        allBooks[existingBookIndex] = book;
        eventBusService.emit('show-msg',{text: "Book updated successfully",type:"success"})
    } else{
        book.id = Date.now().toString();
        allBooks.push(book);
        eventBusService.emit('show-msg',{text: "Book added successfully",type:"success"})
    }
...
function deleteBook(bookId) {
   ...
   eventBusService.emit('show-msg',{text: "Book deleted successfully",type:"error"});
...
function addReview(bookId, review) {
    let allBooks = getBooks();
    const book = allBooks.find(book => bookId === book.id);
    if(!book) return;
...
function deleteReview(bookId, reviewId) {
    ...
    book.reviews = book.reviews.filter(review => reviewId !== review.id);
    localStorage.setItem("books", JSON.stringify(allBooks));
}